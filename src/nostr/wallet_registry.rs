...
impl WalletIndex {
    ...
    pub fn subscribe(&mut self, conn_id: u32, sub_id: String, filters: Vec<Filter>) {
    ...
    pub fn unsubscribe(&mut self, conn_id: u32, sub_id: String) {
    ...
    pub fn save(&self, conn_id: u32) -> Option<SavedState> {
    ...
    pub fn restore(&mut self, conn_id: u32, data: serde_json::Value) {
...
/// A deep module that encapsulates indexing and asynchronous persistence.
...
impl<S: Storage> WalletRegistry<S> {
    ...
    pub async fn disconnect(&mut self, conn_id: u32) {
...
mod tests {
    ...
    fn test_index_matching_grouped() {
        let mut index = WalletIndex::new();

        index.subscribe(1, "sub1".into(), vec![Filter {
            authors: Some(vec!["alice".into()]),
            ..Default::default()
        }]);
        index.subscribe(2, "sub1".into(), vec![Filter {
            authors: Some(vec!["alice".into()]),
            ..Default::default()
        }]);

        let event_alice = make_event("alice", 1, vec![]);
        let matches: Vec<_> = index.match_event(&event_alice).collect();
    ...
    fn test_info_event_caching() {
        ...
        index.cache_info(event.clone());

        let stored = index.get_info("alice");
